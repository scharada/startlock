...
bool __stdcall Lockdown(TCHAR *windowText)
{
	...
	if(!bLockedDown){
		...
		//MoveWindow(hWnd, 0,0, screenX,screenY, TRUE);  // Expand to use the entire screen, this will not hide the menu bar!
		SetWindowPos(hWnd, HWND_TOPMOST, 0, 0, screenX, screenY, SWP_SHOWWINDOW);	//this will not hide the menu bar!
...
bool __stdcall Unlockdown()
{
	...
	if(bLockedDown){
		...
		RECT rectWork;
		SystemParametersInfo(SPI_GETWORKAREA, 0, &rectWork, 0); //{top=36 bottom=640 left=0 right=480}

		MoveWindow(hWndLockdown, rectWork.left, rectWork.top, rectWork.right,rectWork.bottom - menuY, TRUE);  // Expand to use the entire screen
